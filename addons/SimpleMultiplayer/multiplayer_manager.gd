...
export (PackedScene) var network_player_scene
...
func _spawn_player_now(uuid: String, is_local: bool, player_data: Dictionary = {}):
	...
	if player_data.has("x") and player_data.has("y"):
		player.position = Vector2(player_data.get("x", 0), player_data.get("y", 0))
...
func _connect_signals():
	...
	if ws_client:
		if ws_client.has_signal("spawn_local_player"):
			ws_client.connect("spawn_local_player", self, "_on_spawn_local_player")
		if ws_client.has_signal("spawn_new_player"):
			ws_client.connect("spawn_new_player", self, "_on_spawn_new_player")
		if ws_client.has_signal("spawn_network_players"):
			ws_client.connect("spawn_network_players", self, "_on_spawn_network_players")
		if ws_client.has_signal("update_position"):
			ws_client.connect("update_position", self, "_on_update_position")
		if ws_client.has_signal("player_disconnected"):
			ws_client.connect("player_disconnected", self, "_on_player_disconnected")
...
func _on_spawn_local_player(player_data: Dictionary):
	var ws_client = get_node_or_null("/root/WebsocketClient")
	if ws_client and ws_client.has_method("get_player_id"):
		spawn_player(ws_client.get_player_id(), true, player_data)
