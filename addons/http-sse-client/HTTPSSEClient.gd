...
func attempt_to_connect():
	...
	if p == -1:
		p = 443
	var use_ssl = (p == 443)
	var err = httpclient.connect_to_host(domain, p, use_ssl)
	if err == OK:
...
func _process(delta):
	...
	if httpclient.has_response() or httpclient_status == HTTPClient.STATUS_BODY:
		...
		if httpclient_status == HTTPClient.STATUS_BODY:
			...
			while httpclient.get_connection().get_available_bytes():
				var data = httpclient.get_connection().get_partial_data(1)
				if data[0] == OK:
