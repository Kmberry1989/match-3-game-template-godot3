...
func _ready():
	...
	_recalc_start()
	# Recenter grid when viewport size changes (Godot 3-compatible)
	var vp = get_viewport()
	if vp != null:
		...
		if event.is_pressed():
			...
			if is_dragging and dragged_dot != null:
				...
				if difference.length() > 0.5: # Threshold for a swipe
					...
					var swapped = yield(swap_dots(start_grid_pos.x, start_grid_pos.y, direction), "completed")
					if not swapped:
...
func destroy_matches():
		...
		for j in range(height):
			if all_dots[i][j] != null and all_dots[i][j].matched:
				...
				if all_dots[i][j].float_tween:
					all_dots[i][j].float_tween.stop_all()
				if all_dots[i][j].pulse_tween:
					all_dots[i][j].pulse_tween.stop_all()
	...
	if points_earned > 0:
		...
		if match_count > 0:
			...
			# MatchLabel extends Control; use rect_global_position, converting from local grid space
	...
	for i in range(width):
		for j in range(height):
			if all_dots[i][j] == null and not restricted_fill(Vector2(i,j)):
				...
				# Dot.move() returns a Tween (Godot 3), which emits 'tween_all_completed'
...
func ensure_moves_available(max_attempts = 10):
	# Always yield at least once so callers can safely `yield(..., "completed")`
	yield(get_tree(), "idle_frame")
	var attempts = 0
