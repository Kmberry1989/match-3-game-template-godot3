...
onready var music_slider = $Center/VBox/MusicVolume/MusicSlider
onready var music_percent = $Center/VBox/MusicVolume/MusicPercent
...
func _ready():
	...
	shop_button.connect("pressed", self, "_on_shop_toggle")
	buy_frame2.connect("pressed", self, "_on_buy_frame2")
...
func _populate_frame_shop():
	...
	for n in frames:
		...
		btn.connect("pressed", self, "_on_pause_shop_frame_pressed", [frame_name, unlocked])
		shop_panel.add_child(btn)

func _on_pause_shop_frame_pressed(frame_name, was_unlocked):
	if not was_unlocked:
