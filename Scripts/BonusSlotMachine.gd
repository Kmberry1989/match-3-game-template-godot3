...
var _symbol_size: Vector2 = SYMBOL_SIZE
...
export var reel1_textures: Array = []
export var reel2_textures: Array = []
export var reel3_textures: Array = []
...
func _ready() -> void:
	...
	var imgs: Array = []
	for d in _symbols:
		var tex: Texture = d.get("tex", null)
		if tex != null:
			imgs.append(tex)
...
func _build_reel(reel: Control) -> void:
	...
	track.name = "Track"
	track.rect_position = Vector2.ZERO
...
func _make_symbol_tile(sym: Dictionary) -> Control:
	...
	tile.rect_min_size = Vector2(_symbol_size)
	...
	tile.add_stylebox_override("panel", sb)
	# Store a reference to the stylebox for later tweaks in Godot 3 (no get_theme_stylebox)
	tile.set_meta("tile_stylebox", sb)
	var tex: Texture = sym.get("tex", null)
...
func set_reel_symbol_textures(reel_index: int, textures: Array) -> void:
	...
	if track.get_child_count() > max_count:
		...
		var first_tex: Texture = textures[0] if textures.size() > 0 else null
		_apply_texture_to_tile(last_tile, first_tex)

func _apply_texture_to_tile(tile: Control, tex: Texture) -> void:
	...
	var sb = null
	if tile.has_meta("tile_stylebox"):
		sb = tile.get_meta("tile_stylebox")
...
func _on_SpinButton_pressed() -> void:
	...
	if _yielding_ack and not _spinning and not _finished:
...
func _spin_reel_cascade(reel_index: int) -> void:
	...
	track.rect_position.y = 0.0
	# Fast loops
	var loops = 3 + reel_index
	var loop_dur = 0.24
	for _k in range(loops):
		var t1 = create_tween()
		t1.tween_property(track, "rect_position:y", -total_h, loop_dur).set_trans(Tween.TRANS_LINEAR)
		yield(t1, "finished")
	...
	var t2 = create_tween()
	t2.tween_property(track, "rect_position:y", final_y, decel_dur).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
	yield(t2, "finished")
	# Snap to exact boundary
	var snapped_steps: int = int(round(-track.rect_position.y / tile_h))
	track.rect_position.y = -float(snapped_steps) * tile_h
	if AudioManager != null:
...
func _current_top_index(reel_index: int) -> int:
	var track: VBoxContainer = _reel_tracks[reel_index]
	var tile_h: float = float(_symbol_size.y)
	var steps: int = int(round(-track.rect_position.y / tile_h))
...
func _pick_symbol_index() -> int:
	var weights: Array = [25, 20, 10, 8, 8, 12, 5, 12]
	var total: int = 0
	for w in weights:
		total += w
...
func _spin_reel(reel: Control, stop_index: int, duration: float) -> void:
	...
	track.rect_position = Vector2(0, 0)
	...
	tick.start()
	var t = create_tween()
	t.tween_property(track, "rect_position:y", final_y, duration).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	yield(t, "finished")
	track.rect_position = Vector2(0, -tile_h * float(stop_index))
	if tick != null:
		tick.stop()
		tick.queue_free()
	if AudioManager != null:
...
func _majority_symbol(ids: Array) -> int:
	if ids[0] == ids[1]:
		return ids[0]
	if ids[1] == ids[2]:
		return ids[1]
...
func _apply_assets() -> void:
	...
	if File.new().file_exists(bg_path):
		var bg_tex: Texture = load(bg_path) as Texture
		var bg: TextureRect = get_node_or_null("Background") as TextureRect
		if bg != null and bg_tex != null:
			bg.texture = bg_tex
			bg.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_COVERED
	if File.new().file_exists(frame_path):
		var fr_tex: Texture = load(frame_path) as Texture
		var frame: TextureRect = get_node_or_null("Frame") as TextureRect
		if frame != null and fr_tex != null:
			frame.texture = fr_tex
			frame.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	if File.new().file_exists(btn_path):
		var btex: Texture = load(btn_path) as Texture
		if _spin_button != null and btex != null:
			...
			if tb != null:
				...
				if File.new().file_exists(b_hover_path):
					var bhov: Texture = load(b_hover_path) as Texture
					tb.texture_hover = bhov
				else:
					tb.texture_hover = btex
				if File.new().file_exists(b_press_path):
					var bprs: Texture = load(b_press_path) as Texture
					tb.texture_pressed = bprs
				else:
					tb.texture_pressed = btex
				tb.rect_min_size = btex.get_size()
			else:
				var btn: Button = _spin_button as Button
				if btn != null:
					btn.icon = btex
					btn.text = "SPIN"
					# In Godot 3, icon alignment API differs; keep default alignment.
					btn.rect_min_size = btex.get_size()
	if File.new().file_exists(glow_path):
		var gtex: Texture = load(glow_path) as Texture
		for g in _glows:
			if g == null:
				continue
			g.texture = gtex
			...
			for e in exts:
				if lower.ends_with(e):
					...
					if not joined.ends_with("/"):
						joined += "/"
					result[lower] = joined + fn
					break
		fn = d.get_next()
	d.list_dir_end()
	return result

func _load_first_match_tex(index: Dictionary, bases: Array) -> Texture:
	var exts: Array = [".png",".jpg",".jpeg"]
	for b in bases:
		...
		for e in exts:
			...
			if index.has(key):
				var tex: Texture = load(index[key]) as Texture
				if tex != null:
					return tex
	return null
...
func _show_reel_glows(mask: Array) -> void:
	...
	for i in range(min(3, mask.size())):
		...
		g.anchor_right = 0.0
		g.anchor_bottom = 0.0
		g.rect_position = Vector2(0.0, result_row_top)
		g.rect_size = Vector2(row_w, row_h)
		...
		if target > 0.0:
			g.rect_scale = Vector2.ONE
			var p = create_tween()
			p.tween_property(g, "rect_scale", Vector2(1.12, 1.12), 0.18).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
			p.tween_property(g, "rect_scale", Vector2.ONE, 0.18).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
...
func _make_placeholder_tex(_label: String, base: Color, save_path: String) -> Dictionary:
	...
	if save_path != "":
		...
		if placeholder_overwrite_existing or not File.new().file_exists(save_path):
...
func _ensure_placeholder_dir_exists(path: String) -> void:
	...
	if dir_path.begins_with("user://"):
		...
		dd.make_dir_recursive(abs_path)
...
func _spawn_confetti_at(origin: Vector2, intensity: float) -> void:
	...
	for j in range(count):
		...
		var w: float = rand_range(6.0, 12.0)
		var h: float = rand_range(3.0, 8.0)
		cr.rect_size = Vector2(w, h)
		cr.rect_pivot_offset = cr.rect_size * 0.5
		var jitter: Vector2 = Vector2(rand_range(-8.0, 8.0), rand_range(-8.0, 8.0))
		cr.rect_position = origin + jitter - cr.rect_pivot_offset
		if cr.has_method("set_z_index"):
			cr.set_z_index(20)
		add_child(cr)
		var base_ang: float = -PI * 0.5
		var spread: float = PI * 0.7
		var ang: float = base_ang + rand_range(-spread * 0.5, spread * 0.5)
		var dist: float = rand_range(180.0, 420.0) * intensity
		var dir: Vector2 = Vector2(cos(ang), sin(ang))
		var rise: float = rand_range(80.0, 160.0)
		var fall: float = rand_range(160.0, 260.0)
		var peak: Vector2 = origin + dir * (dist * 0.55) + Vector2(0.0, -rise)
		var target: Vector2 = origin + dir * dist + Vector2(0.0, fall)
		var dur_up: float = rand_range(0.28, 0.38)
		var dur_down: float = rand_range(0.42, 0.58)
		var total_dur: float = dur_up + dur_down
		var rot: float = rand_range(-7.0, 7.0)
		var t_move = create_tween()
		t_move.tween_property(cr, "rect_position", peak - cr.rect_pivot_offset, dur_up).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		t_move.tween_property(cr, "rect_position", target - cr.rect_pivot_offset, dur_down).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
		var t_rot = create_tween()
		t_rot.tween_property(cr, "rect_rotation", rot, total_dur).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		var t_scale = create_tween()
		t_scale.tween_property(cr, "rect_scale", Vector2(1.06, 1.06), dur_up * 0.8).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		t_scale.tween_property(cr, "rect_scale", Vector2(1.0, 1.0), dur_down).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
		...
		t_fade.connect("finished", self, "_on_tween_fade_finished", [cr])
...
func _on_tween_fade_finished(cr):
...
func _layout_for_viewport() -> void:
	...
	if panel != null:
		...
		panel.margin_left = -panel_w * 0.5
		panel.margin_right = panel_w * 0.5
		panel.margin_top = -panel_h * 0.5
		panel.margin_bottom = panel_h * 0.5
	...
	_symbol_size = Vector2(target, target)
...
func _animate_in() -> void:
	...
	if panel != null:
		...
		t.parallel().tween_property(panel, "rect_scale", Vector2.ONE, 0.25).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	if dimmer != null:
...
func _animate_out() -> void:
	...
	if panel != null:
		...
		t.parallel().tween_property(panel, "rect_scale", Vector2(0.94, 0.94), 0.25).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	if dimmer != null:
