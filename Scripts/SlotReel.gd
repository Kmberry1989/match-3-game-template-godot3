...
func setup_reel(p_images: Array, p_rows_visible: int, p_tile_size: Vector2, p_spin_up: float) -> void:
	images = p_images
...
func _assign_random_texture(spr):
	if images.size() == 0:
		spr.texture = null
		spr.set_meta("idx", -1)
		return
...
func start_spin(runtime_s: float, speed_tiles_per_sec: float, delay_s: float = 0.0) -> void:
	yield(get_tree().create_timer(max(delay_s, 0.0)), "timeout")
	yield(_spin_up(), "completed")
	...
	for _i in range(moves):
		yield(_step_down(step_time), "completed")
		_wrap_and_randomize_if_needed()
	yield(_spin_down(), "completed")
...
func _spin_up() -> void:
	...
	if _anim.has_animation("spin_up"):
		_anim.remove_animation("spin_up")
	_anim.add_animation("spin_up", anim)
	_anim.play("spin_up")
...
func _spin_down() -> void:
	...
	if _anim.has_animation("spin_down"):
		_anim.remove_animation("spin_down")
	_anim.add_animation("spin_down", anim)
